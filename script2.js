...
window.updatePaperRate = function() {
  ...
  // Only auto-fill if a type is selected, but allow the user to change it after
  document.getElementById('rate').value = paperRates[paperType] || '';
};
// --- Paper Type and Rate Logic ---
const paperRates = {
  'Artpaper': 78,
  'ArtCard': 78,
  'A Maplitho': 76,
...
function getBindingBaseCost() {
  const bindingCostElem = document.getElementById('binding-cost');
  if (bindingCostElem && bindingCostElem.textContent) {
    const match = bindingCostElem.textContent.match(/([\d,.]+)/);
